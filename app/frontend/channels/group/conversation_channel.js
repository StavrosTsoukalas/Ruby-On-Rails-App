...
function subToGroupConversationChannel(id) {
    consumer['group_conversation_' + id]  = consumer.subscriptions.create(
        {
...
$(document).on('submit', '.send-group-message', function(e) {
    ...
    consumer['group_conversation_' + id].send_message(message_values);
});
...
$(document).on('click', '.conversation-window, .group-conversation', function(e) {
    ...
    if (unseen_by_user) {
        ...
        consumer['group_conversation_' + conv_id].set_as_seen(conv_id);
    }
});

window.MarkGroupConvAsUnseen = function(message_user_id, conversation_id) {
...
window.modifyConversationsMenuList = function(conversation_id) {
