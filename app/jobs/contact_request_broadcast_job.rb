class ContactRequestBroadcastJob < ApplicationJob
  ...
  def perform(contact_request)
    ...
    ActionCable.server.broadcast("notifications_#{receiver.id}",{notification: 'contact-request-received', sender_name: sender.name,contact_request: render_contact_request(sender, contact_request)})
  end
