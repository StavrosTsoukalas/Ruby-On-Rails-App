class Private::MessageBroadcastJob < ApplicationJob
  ...
  def broadcast_to_sender(sender, recipient, message, previous_message)
    ActionCable.server.broadcast("private_conversations_#{sender.id}", {message: render_message(message, previous_message, sender), conversation_id: message.conversation_id, recipient_info: recipient})
  end

  def broadcast_to_recipient(sender, recipient, message, previous_message)
    ActionCable.server.broadcast("private_conversations_#{recipient.id}", {recipient: true, sender_info: sender, message: render_message(message, previous_message, recipient), conversation_id: message.conversation_id})
  end
