class Group::MessageBroadcastJob < ApplicationJob
  ...
  def perform(message, previous_message, current_user)
    ...
    ActionCable.server.broadcast("group_conversation_#{conversation_id}", {message: render_message(message, previous_message), conversation_id: conversation_id, user_id: message.user_id})
  end
